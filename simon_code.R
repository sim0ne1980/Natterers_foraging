...
getwd()
#changed to package sf instead of sp
bats.polys<-readOGR("data/All_stHildas.shp", layer="All_stHildas")
bats.polyswall<-readOGR("data/wallandlc.shp", layer="wallandlc")
str(bats.polys)
...
bats<-unique(bats.polys@data$MERGE_SRC)
for(bat in 1:length(bats)){
  ...
  for(i in 1:iter){
    iter.points<-list()
    iter.clusters<-list()
    bat.polys<-bats.polys[which(bats.polys@data$MERGE_SRC==bats[bat]),]
    for(poly in 1:nrow(bat.polys)){
      ...
      poly.data<-data.frame("BMERGR_SRC"=rep(bats[bat],pts_per_poly),"Date"=rep(poly.date,pts_per_poly),"Time"=rep(poly.time,pts_per_poly),"RepID"=rep(i,pts_per_poly),"PtID"=seq(1,pts_per_poly))
