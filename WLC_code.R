...
bats.polys1<-readOGR("./data/wallandlc.shp", layer="wallandlc")
...
bats<-unique(bats.polys1@data[,"Bat_name"])
for(bat in 1:length(bats)){
  ...
  for(i in 1:iter){
    iter.points<-list()
    iter.clusters<-list()
    bat.polys<-bats.polys1[which(bats.polys1@data[,"Bat_name"]==bats[bat]),]
...
writeOGR(bats.points,"./output/WLC_points.shp",layer="points",driver="ESRI Shapefile")
writeOGR(bats.clusters,"./output/WLC_clusters.shp",layer="clusters",driver="ESRI Shapefile")
writeOGR(bats.intersections,"./output/WLC_intersections.shp",layer="intersections",driver="ESRI Shapefile")
